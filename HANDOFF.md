...
- MVP 主流程完整可用（注册登录 -> 词卡 -> 学习训练 -> 错题 -> 打卡积分 -> 排行榜）
- 学习统计与仪表盘已落地（统计页可用）
- 用户资料与学习计划已落地
...
- 新用户注册后立即同步排行榜 ZSet（注册即可见排名）
- 应用启动时自动从 MySQL 重建排行榜缓存（Redis 重启后可自动恢复）
- 排行榜 getTopUsers 从 N+1 查询改为批量查询

**Redis 随机题池：**

- 键格式：`wordsprint:pool:{userId}`，使用 Redis Set 存储
- 首次请求从 MySQL 加载题池并写入 Redis，TTL 1 小时
- 后续请求从 Redis Set 中取随机子集，批量查询 WordCard
- 词卡增/删/改/CSV 导入时自动清除该用户题池缓存，保证数据一致性
- 重复提交保护：`wordsprint:submit:lock:{userId}:{cardId}:{mode}:{fingerprint}`，SETNX + 3s TTL

**关键业务日志：**

- `AuthServiceImpl`：注册成功、登录成功
- `StudyServiceImpl`：学习提交
- `WrongWordServiceImpl`：错题移除、错题恢复
- `WordCardServiceImpl`：词卡创建、更新、删除、CSV 导入
- `PublicWordServiceImpl`：公共词库导入、管理员 CSV 导入
- `RedisRankServiceImpl`：启动时缓存重建
...
- 学习统计页（范围切换、ECharts 趋势图、熟练度环形图、打卡日历）
- 个人中心（资料更新 + 学习计划设置）
...
结合 `ITERATION_TODO.md`，所有 MVP 和阶段 12 已完成。无剩余必做项。

### 可选后续增强

- ECharts 按需加载可进一步优化（目前仅统计页使用，已做 dynamic import）

---

## 6. 下一步推荐顺序

所有阶段 12 完善项已完成。项目已进入可交付状态。
