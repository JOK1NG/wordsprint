...
当前仓库已补齐规划文档，并完成了第一批可运行骨架与联调验证。

### 已完成

- 项目文档已补齐：`AGENTS.md`、`README.md`、`WORD_CARD_DEV_PLAN.md`、`FRONTEND_PAGE_DESIGN.md`、`DATABASE_DESIGN.md`、`API_LIST.md`、`ITERATION_TODO.md`
- 前后端项目骨架已初始化
- `sql/init.sql` 已生成并验证可执行
- 本地 `MySQL` 与 `Redis` 启动方式已写入文档
- 后端注册 / 登录 / 当前用户接口已完成
- 前端登录 / 注册页已接入接口
- 后端单词卡 CRUD 骨架已完成
- 后端健康检查、注册 / 登录 / 当前用户、单词卡 CRUD 已完成真实联调
- 前端依赖安装和生产构建已通过
...
- 后端鉴权和单词卡 CRUD 后端已完成并做过真实接口验证
- 当前主要缺口在：单词卡前端页面、学习训练主流程、错题本、排行榜、统计
